...
$pc_name = '';
$pc_status = '';
$pc_updated_by = getSession('admin_id');
$flag = 0;
if (isset($_GET['id'])) {
    $pc_id = $_GET['id'];
}
if (isset($_POST['pc_name'])) {
    ...
    $pc_id = validateInput($pc_id);
    $pc_name = validateInput($pc_name);
    $pc_status = validateInput($pc_status);

    // check exists
    $sql_check = "SELECT * FROM product_category WHERE pc_name='$pc_name' AND pc_id NOT IN (" . $pc_id . ")";
    ...
    if ($count > 0) {
        ...
        if ($flag == 0) {
            ...
            $custom_array .= ',pc_updated_by = "' . $pc_updated_by . '"';

            $sql = "UPDATE product_category SET $custom_array WHERE pc_id = $pc_id";
...
$sqlData = "SELECT * FROM product_category WHERE pc_id = $pc_id";
$resultData = mysqli_query($con, $sqlData);
if ($resultData) {
    $obj = mysqli_fetch_object($resultData);
    $pc_name = $obj->pc_name;
    $pc_status = $obj->pc_status;
} else {
...
<html>
    <head>
        ...
        <title>Alook Refrigeration CO. Ltd | Edit Product Category</title>
